...
cancer_data <- read_csv("raw_data/opendata_inc9620_hb.csv") %>% 
  janitor::clean_names()

# Filter for NHS Borders data 
# Remove columns containing statistical qualifiers and EASR and WASR data as 
# we won't be using this for analysis
borders_cancer_data <- cancer_data %>% 
  filter(hb == "S08000016") %>% 
  select(-ends_with("qf"), -starts_with("easr"), -starts_with("wasr"))
...
subset_borders_cancer_data <- borders_cancer_data %>% 
...
write_csv(subset_borders_cancer_data, "clean_data/borders_cancer_data.csv")
